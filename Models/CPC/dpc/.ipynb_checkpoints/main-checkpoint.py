...
def main():
    ...
    if args.dataset == 'ucf101': # designed for ucf101, short size=256, rand crop to 224x224 then scale to 128x128
        transform = transforms.Compose([
            ...
            RandomGray(consistent=False, p=0),
            ColorJitter(brightness=0, contrast=0, saturation=0, hue=0, p=0),
            ToTensor(),
            Normalize()
        ])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for name, param in model.named_parameters():
            if ('bn' not in name) and (param.requires_grad) and ("bias" not in name):            
                writer_train.add_histogram(name, param.grad.abs().mean(), epoch)
...
def train(data_loader, model, optimizer, epoch):
    ...
    for idx, input_seq in enumerate(data_loader):
        ...
        ave_grads = get_grad_flow(model.named_parameters())
    ...
    return losses.local_avg, accuracy.local_avg, [i.local_avg for i in accuracy_list], ave_grads
