...
def main_CatCam(f_root, splits_root, csv_root='../data/catcam/'):
    '''generate training/testing split, count number of available frames, save in csv'''
    if not os.path.exists(csv_root): os.makedirs(csv_root)
    for which_split in [1,2,3]:
        train_set = []
        ...
        with open(train_split_file, 'r') as f:
            ...
            for line in f:
                vpath = os.path.join(f_root, line.split(' ')[0][0:-4]) + '/'
                train_set.append([vpath, len(glob.glob(os.path.join(vpath, '*.tif')))])
        ...
        with open(test_split_file, 'r') as f:
            for line in f:
                vpath = os.path.join(f_root, line.rstrip()[0:-4]) + '/'
                test_set.append([vpath, len(glob.glob(os.path.join(vpath, '*.tif')))])
...
if __name__ == '__main__':
    ...
    # edit 'your_path' here: 
    # F = os.getenv('SLURM_TMPDIR') + '/UCF101catcam/frame'
    # S = os.getenv('SLURM_TMPDIR') + '/UCF101catcam/splits_classification'
    # main_CatCam(f_root=F, 
    #            splits_root=S)
   F = os.getenv('SLURM_TMPDIR') + '/UCF101/frame'
   S = os.getenv('SLURM_TMPDIR') + '/UCF101/splits_classification'
   main_UCF101(f_root=F, 
            splits_root=S)


    # main_HMDB51(f_root='/Users/shahab/Mila/Data/HMDB51/frame',
    #           splits_root='/Users/shahab/Mila/Data/HMDB51/split/testTrainMulti_7030_splits')
